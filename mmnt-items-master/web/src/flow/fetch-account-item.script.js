...
pub fun fetch(address: Address, id: UInt64): AccountItem? {
  if let col = getAccount(address).getCapability<&MmntItems.Collection{NonFungibleToken.CollectionPublic, MmntItems.MmntItemsCollectionPublic}>(MmntItems.CollectionPublicPath).borrow() {
    if let item = col.borrowMmntItem(id: id) {
