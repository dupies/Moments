...
const momentPath = '"../../contracts/Moment.cdc"';
const mmntItemsPath = '"../../contracts/MmntItems.cdc"';
const mmntItemsMarkPath = '"../../contracts/MmntItemsMarket.cdc"';

class MarketService {
  constructor(
    private readonly flowService: FlowService,
    private readonly fungibleTokenAddress: string,
    private readonly momentAddress: string,
  ...
  setupAccount = () => {
    ...
    const transaction = fs
      .readFileSync(
        path.join(
          __dirname,
          `../../../cadence/transactions/mmntItemsMarket/setup_account.cdc`
        ),
        "utf8"
      )
      .replace(mmntItemsMarkPath, fcl.withPrefix(this.marketAddress));
  ...
  getItem = (account: string, itemID: number) => {
    const script = fs
      .readFileSync(
        path.join(
          __dirname,
          `../../../cadence/cadence/mmntItemsMarket/scripts/read_collection_ids.cdc`
        ),
        "utf8"
      )
      .replace(mmntItemsMarkPath, fcl.withPrefix(this.marketAddress));
  ...
  buy = (account: string, itemID: number) => {
    ...
    const transaction = fs
      .readFileSync(
        path.join(
          __dirname,
          `../../../cadence/transactions/mmntItemsMarket/buy_market_item.cdc`
        ),
        "utf8"
      )
      ...
      .replace(momentPath, fcl.withPrefix(this.momentAddress))
  ...
  sell = (itemID: number, price: number) => {
    ...
    const transaction = fs
      .readFileSync(
        path.join(
          __dirname,
          `../../../cadence/transactions/mmntItemsMarket/sell_market_item.cdc`
        ),
        "utf8"
      )
      .replace(fungibleTokenPath, fcl.withPrefix(this.fungibleTokenAddress))
