...
class MomentsService {
  ...
  setupAccount = async () => {
    ...
    const transaction = fs
      .readFileSync(
        path.join(
          __dirname,
          "../../../cadence/transactions/moment/setup_account.cdc"
        ),
        "utf8"
      )
  ...
  mint = async (recipient: string, amount: number) => {
    ...
    const transaction = fs
      .readFileSync(
        path.join(
          __dirname,
          "../../../cadence/transactions/moment/mint_tokens.cdc"
        ),
        "utf8"
      )
      .replace(fungibleTokenPath, fcl.withPrefix(this.fungibleTokenAddress))
  ...
  burn = async (amount: number) => {
    ...
    const transaction = fs
      .readFileSync(
        path.join(
          __dirname,
          "../../../cadence/transactions/moment/burn_tokens.cdc"
        ),
        "utf8"
      )
      .replace(fungibleTokenPath, fcl.withPrefix(this.fungibleTokenAddress))
  ...
  getBalance = async (account: string) => {
    const script = fs
      .readFileSync(
        path.join(__dirname, "../../../cadence/scripts/moment/get_balance.cdc"),
        "utf8"
      )
      .replace(fungibleTokenPath, fcl.withPrefix(this.fungibleTokenAddress))
      .replace(momentPath, fcl.withPrefix(this.momentAddress));
  ...
  getSupply = async () => {
    const script = fs
      .readFileSync(
        path.join(__dirname, "../../../cadence/scripts/moment/get_supply.cdc"),
        "utf8"
      )
      .replace(fungibleTokenPath, fcl.withPrefix(this.fungibleTokenAddress))
      .replace(momentPath, fcl.withPrefix(this.momentAddress));
