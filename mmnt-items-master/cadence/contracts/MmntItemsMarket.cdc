import Moment from "./Moment.cdc"
import MmntItems from "./MmntItems.cdc"
import FungibleToken from "./FungibleToken.cdc"
import NonFungibleToken from "./NonFungibleToken.cdc"

/*
    This is a simple MmntItems initial sale contract for the DApp to use
    in order to list and sell MmntItems.
...
pub contract MmntItemsMarket {
    ...
    // SaleOffer
    // A MmntItems NFT being offered to sale for a set fee paid in Moment.
    //
    pub resource SaleOffer: SaleOfferPublicView {
        ...
        access(self) let sellerItemProvider: Capability<&MmntItems.Collection{NonFungibleToken.Provider}>

        // The Moment vault that will receive that payment if teh sale completes successfully.
        access(self) let sellerPaymentReceiver: Capability<&Moment.Vault{FungibleToken.Receiver}>

        // Called by a purchaser to accept the sale offer.
        // If they send the correct payment in Moment, and if the item is still available,
        // the MmntItems NFT will be placed in their MmntItems.Collection .
        //
        pub fun accept(
            buyerCollection: &MmntItems.Collection{NonFungibleToken.Receiver},
        ...
        // to transfer the MmntItems NFT and the capability to receive Moment in payment.
        //
        init(
            sellerItemProvider: Capability<&MmntItems.Collection{NonFungibleToken.Provider, MmntItems.MmntItemsCollectionPublic}>,
            itemID: UInt64,
            typeID: UInt64,
            sellerPaymentReceiver: Capability<&Moment.Vault{FungibleToken.Receiver}>,
            price: UFix64
        ) {
            pre {
            ...
            assert(
                collectionRef.borrowMmntItem(id: itemID) != nil,
    ...
    pub resource interface CollectionManager {
        pub fun insert(offer: @MmntItemsMarket.SaleOffer)
    ...
    pub resource Collection : CollectionManager, CollectionPurchaser, CollectionPublic {
        ...
        // purchase
        // If the caller passes a valid itemID and the item is still for sale, and passes a Moment vault
        // typed as a FungibleToken.Vault (Moment.deposit() handles the type safety of this)
        // containing the correct payment amount, this will transfer the MmntItem to the caller's
        // MmntItems collection.
    ...
    init () {
        ...
        self.CollectionStoragePath = /storage/mmntItemsMarketCollection002
        self.CollectionPublicPath = /public/mmntItemsMarketCollection002
    }
}
